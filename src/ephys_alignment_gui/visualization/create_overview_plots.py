...
def make_overview_plot(folder, sess_info, save_folder=None) -> None:
    image_folder = folder
    ...
    img_files_sort = [
        next((file for file in img_files if keyword in file), None)
    ...
    for iF, file in enumerate(img_files_sort):
        ...
        ax = fig.add_subplot(
            gs[
                img_row_order[iF],
                img_column_order[iF] : img_column_order[iF] + 3,
            ]
        )
    ...
    probe_line_files_sort = [
        next((file for file in probe_line_files if keyword in file), None)
        for keyword in probe_line_keywords
    ]
    ...
    slice_files_sort = [
        next((file for file in slice_files if keyword in file), None)
    ...
    slice_files_zoom = glob.glob(
        str(image_folder.joinpath(image_info + "slice_zoom*.png"))
    )
    slice_row_order = [0, 1, 2]
    ...
    slice_files_sort = [
        next((file for file in slice_files_zoom if keyword in file), None)
        for keyword in slice_keywords
    ]
    ...
    fig.savefig(
        save_folder.joinpath(image_info + "overview.png"),
        pad_inches=0.1,
        dpi=fig.dpi,
        bbox_inches="tight",
    )
